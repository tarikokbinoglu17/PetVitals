...
function demoAnswer(petId: string, question: string) {
  const turkish =
    /[çğıöşüÇĞİÖŞÜ]|\b(nasıl|sağlık|aşı|ilaç|kilo|veteriner|geçmiş|moka|luna)\b/i.test(
      question,
    );
  if (petId === "1") {
    return turkish
      ? "Moka’nın kayıtlı sağlık geçmişinde 12 Ağustos 2026’da parazit tablet uygulaması ve 2 Eylül 2026 için planlanan karma aşı tekrarı görünüyor. Kayıtlarda şu anda acil bir risk işareti yok. Veteriner görüşmesinde yaklaşan karma aşının zamanlamasını, parazit korumasının devam planını ve güncel kilo kontrolünü sorabilirsiniz."
      : "Moka’s recorded history shows a parasite treatment on 12 Aug 2026 and a combination-vaccine booster planned for 2 Sep 2026. No urgent risk signal is visible in the demo history. Ask the veterinarian about booster timing, parasite prevention and an updated weight check.";
  }
  return turkish
    ? "Luna’nın kayıtlı sağlık geçmişinde 18 Eylül 2026 için rutin veteriner kontrolü planlanmış görünüyor. Mevcut demo kayıtlarında acil bir risk işareti yok. Kontrolde genel muayene, kilo takibi, aşı takvimi ve parazit koruma planını gözden geçirmek uygun olur."
    : "Luna’s recorded history shows a routine veterinary checkup planned for 18 Sep 2026. No urgent risk signal is visible in the demo history. Review general exam findings, weight, vaccination schedule and parasite prevention at the visit.";
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (petId === "1" || petId === "2") {
      return jsonResponse({
        answer: demoAnswer(petId, question),
        disclaimer:
          "PetVitals AI provides educational support, not a veterinary diagnosis.",
      });
    }
