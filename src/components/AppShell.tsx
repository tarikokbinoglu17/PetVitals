...
import { PetCookieGoIntelligencePanel } from './PetCookieGoIntelligencePanel';

const tabCopy = {
  tr: ['Bugün', 'Dostlarım', 'Sağlık', 'Yaşam', 'Yakınımda', 'PetCookieGo+', 'Profil'],
  en: ['Today', 'My Pets', 'Health', 'Life', 'Nearby', 'PetCookieGo+', 'Profile'],
  de: ['Heute', 'Tiere', 'Gesundheit', 'Leben', 'In der Nähe', 'PetCookieGo+', 'Profil'],
  es: ['Hoy', 'Mascotas', 'Salud', 'Vida', 'Cerca', 'PetCookieGo+', 'Perfil'],
  ja: ['今日', 'ペット', '健康', '生活', '周辺', 'PetCookieGo+', 'プロフィール'],
} as const;
const statusCopy = {
  tr: { demo: 'Demo sürümü · Süre sınırı yok', checking: 'Üyelik durumunuz kontrol ediliyor…', loading: 'Bilgileriniz yükleniyor…', failed: 'Bilgiler yüklenemedi', trial: (days:number) => `Premium deneme · ${days} gün kaldı` },
  en: { demo: 'Demo · No time limit', checking: 'Checking your membership…', loading: 'Loading your information…', failed: 'Could not load information', trial: (days:number) => `Premium trial · ${days} days left` },
  de: { demo: 'Demo · Ohne Zeitlimit', checking: 'Mitgliedschaft wird geprüft…', loading: 'Daten werden geladen…', failed: 'Daten konnten nicht geladen werden', trial: (days:number) => `Premium-Test · ${days} Tage übrig` },
  es: { demo: 'Demo · Sin límite de tiempo', checking: 'Comprobando tu membresía…', loading: 'Cargando tu información…', failed: 'No se pudo cargar la información', trial: (days:number) => `Prueba Premium · quedan ${days} días` },
  ja: { demo: 'デモ · 時間制限なし', checking: 'メンバーシップを確認しています…', loading: '情報を読み込んでいます…', failed: '情報を読み込めませんでした', trial: (days:number) => `Premium無料体験 · 残り${days}日` },
} as const;
...
export function AppShell({ demoMode, userId }: { demoMode: boolean; userId?: string }) {
  ...
  const screen = (() => {
    ...
    if (tab === 'pets') return selectedPet ? <PetDetailScreen onBack={() => { setSelectedPetId(null); scrollToTop(); }} onDeletePet={() => deletePet(selectedPet.id)} onDeleteRecord={deleteRecord} onUpdatePet={draft => updatePet(selectedPet.id, draft)} pet={selectedPet} records={records} savingPet={savingPet} /> : <PetsScreen onAddPet={addPet} onSelectPet={pet => { setSelectedPetId(pet.id); scrollToTop(); }} pets={pets} savingPet={savingPet} />;
