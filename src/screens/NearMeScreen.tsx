...
function EmbeddedMap({ location, places, onSelect }: { location: Coordinates; places: NearbyPlace[]; onSelect: (place: NearbyPlace) => void }) {
  if (Platform.OS === 'web') {
    return <View style={styles.mapFallback}><Text style={styles.mapFallbackText}>Harita mobil uygulamada görüntülenir.</Text></View>;
  }

  try {
    // expo-maps is intentionally loaded only when this screen is used. It requires a development/app build.
      ...
      return (
        <AppleMaps.View
          ...
          onMarkerClick={(event: any) => {
            const place = places.find(item => item.id === event?.id);
    ...
    // Expo Go does not include expo-maps. The list below remains fully usable.
  }

  return (
    <View style={styles.mapFallback}>
      <Text style={styles.mapFallbackTitle}>Harita development build ile açılır</Text>
      <Text style={styles.mapFallbackText}>Yakındaki yerler ve yol tarifi listeden kullanılabilir.</Text>
    </View>
  );
}
...
export function NearMeScreen() {
  ...
  const search = useCallback(async (coords?: Coordinates | null) => {
    ...
    try {
      const result = await findNearbyPetServices({
        ...target,
        radiusMeters: emergencyOnly ? 25000 : 10000,
        category: emergencyOnly ? 'veterinary' : category,
        openNowOnly: emergencyOnly,
        languageCode: 'tr',
      });
      setPlaces(result);
      setSelectedId(result[0]?.id ?? null);
      if (emergencyOnly && result.length === 0) {
  ...
  }, [category, emergencyOnly, location, requestLocation]);

  useEffect(() => {
    requestLocation().then(coords => { if (coords) void search(coords); });
  // Initial location/search only; filter changes are user-controlled via buttons below.
  ...
  const openDirections = async (place: NearbyPlace) => {
    const url = place.mapsUrl || (Platform.OS === 'ios'
      ? `http://maps.apple.com/?daddr=${place.latitude},${place.longitude}&dirflg=d`
  ...
  const callPlace = async (place: NearbyPlace) => {
    ...
    await Linking.openURL(`tel:${place.phone.replace(/[^+\d]/g, '')}`);
  };

  const applyCategory = (next: NearbyCategory) => {
    setEmergencyOnly(false);
    setCategory(next);
    setTimeout(() => void search(location), 0);
  };
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.filters}>
        <Pressable onPress={() => applyCategory('all')} style={[styles.chip, category === 'all' && !emergencyOnly && styles.chipActive]}><Text style={[styles.chipText, category === 'all' && !emergencyOnly && styles.chipTextActive]}>Tümü</Text></Pressable>
        <Pressable onPress={() => applyCategory('veterinary')} style={[styles.chip, category === 'veterinary' && !emergencyOnly && styles.chipActive]}><Text style={[styles.chipText, category === 'veterinary' && !emergencyOnly && styles.chipTextActive]}>Veteriner</Text></Pressable>
        <Pressable onPress={() => applyCategory('petshop')} style={[styles.chip, category === 'petshop' && !emergencyOnly && styles.chipActive]}><Text style={[styles.chipText, category === 'petshop' && !emergencyOnly && styles.chipTextActive]}>Petshop</Text></Pressable>
      </View>
      ...
      <View style={styles.actionRow}>
        <Pressable onPress={() => void search(location)} style={styles.refreshButton}><Text style={styles.refreshText}>↻ Yenile</Text></Pressable>
      ...
      <View style={styles.safetyCard}>
        <Text style={styles.safetyTitle}>Acil durumda saat bilgisini doğrulayın</Text>
        <Text style={styles.safetyText}>Haritadaki “açık” bilgisi işletmenin yayınladığı çalışma saatlerine dayanır ve hatalı veya güncel olmayan bilgi olabilir. Yola çıkmadan önce kliniği arayın.</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  chip: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 999, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 9 },
  chipActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  chipText: { color: colors.text, fontWeight: '800' },
  chipTextActive: { color: colors.white },
  emergencyButton: { backgroundColor: '#FFF2F0', borderColor: '#F0C0BB', borderRadius: 18, borderWidth: 1, marginTop: 13, padding: 15 },
  ...
  actionRow: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginTop: 12 },
  refreshButton: { backgroundColor: colors.primarySoft, borderRadius: 12, paddingHorizontal: 13, paddingVertical: 9 },
  ...
  map: { borderRadius: 20, height: 300, marginTop: 15, overflow: 'hidden', width: '100%' },
  mapFallback: { alignItems: 'center', backgroundColor: colors.primarySoft, borderRadius: 20, height: 210, justifyContent: 'center', marginTop: 15, padding: 24 },
  mapFallbackTitle: { color: colors.primaryDark, fontSize: 16, fontWeight: '900', textAlign: 'center' },
  mapFallbackText: { color: colors.muted, lineHeight: 19, marginTop: 7, textAlign: 'center' },
  selectedCard: { ...shadow, backgroundColor: colors.surface, borderRadius: 20, marginTop: 14, padding: 17 },
  selectedKind: { color: colors.primary, fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  selectedName: { color: colors.text, fontSize: 19, fontWeight: '900', marginTop: 4 },
  ...
  primaryAction: { alignItems: 'center', backgroundColor: colors.primary, borderRadius: 13, flex: 1, paddingVertical: 12 },
  primaryActionText: { color: colors.white, fontWeight: '900' },
  secondaryAction: { alignItems: 'center', backgroundColor: colors.primarySoft, borderRadius: 13, flex: 1, paddingVertical: 12 },
  ...
  placeCard: { alignItems: 'center', backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 16, borderWidth: 1, flexDirection: 'row', marginBottom: 9, padding: 12 },
  placeCardSelected: { borderColor: colors.primary },
  placeIcon: { alignItems: 'center', backgroundColor: colors.primarySoft, borderRadius: 12, height: 40, justifyContent: 'center', width: 40 },
  ...
  placeMeta: { color: colors.primaryDark, fontSize: 11, fontWeight: '700', marginTop: 5 },
  chevron: { color: colors.muted, fontSize: 26, marginLeft: 6 },
  safetyCard: { backgroundColor: '#FFF8E8', borderRadius: 17, marginTop: 12, padding: 15 },
