...
const copy = {
  tr: { demoUser:'Demo Kullanıcı', user:'PetVitals Kullanıcısı', exportReal:'Veri dışa aktarma gerçek hesapta kullanılabilir.', exportFail:'Veriler dışa aktarılamadı. Lütfen tekrar deneyin.', deleteReal:'Hesap silme gerçek hesapta kullanılabilir.', deleteTitle:'Hesabı kalıcı olarak sil?', deleteBody:'PetVitals hesabınız ve hesabınıza bağlı veriler silinecek. Bu işlem geri alınamaz.', cancel:'Vazgeç', delete:'Hesabı sil', deleteFail:'Hesap silinemedi. Lütfen tekrar deneyin.', secure:'Oturumunuz güvenli biçimde saklanıyor.', demo:'Demo modu etkin.', privacy:'PetVitals verilerinizi dışa aktarabilir veya hesabınızı uygulama içinden kalıcı olarak silebilirsiniz.', export:'Verilerimi dışa aktar', deleteForever:'Hesabımı kalıcı olarak sil', metric:'Metrik · kg', imperial:'İngiliz · lb', exportTitle:'PetVitals veri dışa aktarımı' },
  en: { demoUser:'Demo User', user:'PetVitals User', exportReal:'Data export is available for real accounts.', exportFail:'Data could not be exported. Please try again.', deleteReal:'Account deletion is available for real accounts.', deleteTitle:'Delete account permanently?', deleteBody:'Your PetVitals account and related data will be deleted permanently. This cannot be undone.', cancel:'Cancel', delete:'Delete account', deleteFail:'Account could not be deleted. Please try again.', secure:'Your session is stored securely.', demo:'Demo mode is active.', privacy:'You can export your PetVitals data or permanently delete your account from the app.', export:'Export my data', deleteForever:'Delete my account permanently', metric:'Metric · kg', imperial:'Imperial · lb', exportTitle:'PetVitals data export' },
  de: { demoUser:'Demo-Benutzer', user:'PetVitals-Benutzer', exportReal:'Der Datenexport ist für echte Konten verfügbar.', exportFail:'Daten konnten nicht exportiert werden. Bitte erneut versuchen.', deleteReal:'Die Kontolöschung ist für echte Konten verfügbar.', deleteTitle:'Konto dauerhaft löschen?', deleteBody:'Ihr PetVitals-Konto und die zugehörigen Daten werden dauerhaft gelöscht. Dies kann nicht rückgängig gemacht werden.', cancel:'Abbrechen', delete:'Konto löschen', deleteFail:'Konto konnte nicht gelöscht werden. Bitte erneut versuchen.', secure:'Ihre Sitzung wird sicher gespeichert.', demo:'Demo-Modus ist aktiv.', privacy:'Sie können Ihre PetVitals-Daten exportieren oder Ihr Konto in der App dauerhaft löschen.', export:'Meine Daten exportieren', deleteForever:'Mein Konto dauerhaft löschen', metric:'Metrisch · kg', imperial:'Imperial · lb', exportTitle:'PetVitals-Datenexport' },
  es: { demoUser:'Usuario demo', user:'Usuario de PetVitals', exportReal:'La exportación de datos está disponible para cuentas reales.', exportFail:'No se pudieron exportar los datos. Inténtalo de nuevo.', deleteReal:'La eliminación de cuenta está disponible para cuentas reales.', deleteTitle:'¿Eliminar la cuenta permanentemente?', deleteBody:'Tu cuenta de PetVitals y los datos relacionados se eliminarán de forma permanente. Esta acción no se puede deshacer.', cancel:'Cancelar', delete:'Eliminar cuenta', deleteFail:'No se pudo eliminar la cuenta. Inténtalo de nuevo.', secure:'Tu sesión se almacena de forma segura.', demo:'El modo demo está activo.', privacy:'Puedes exportar tus datos de PetVitals o eliminar permanentemente tu cuenta desde la app.', export:'Exportar mis datos', deleteForever:'Eliminar mi cuenta permanentemente', metric:'Métrico · kg', imperial:'Imperial · lb', exportTitle:'Exportación de datos de PetVitals' },
} as const;
...
export function ProfileScreen() {
  ...
  const name = user?.user_metadata?.full_name || (demoMode ? c.demoUser : c.user);

  const handleExport = async () => {
    if (!user?.id || demoMode) { setMessage(c.exportReal); return; }
    setBusy('export'); setMessage(null);
    try { const data = await exportUserData(user.id); await Share.share({ message: data, title: c.exportTitle }); }
    catch { setMessage(c.exportFail); }
    finally { setBusy(null); }
  };

  const handleDelete = () => {
    if (!user?.id || demoMode) { setMessage(c.deleteReal); return; }
    Alert.alert(c.deleteTitle, c.deleteBody, [
      { text: c.cancel, style: 'cancel' },
      { text: c.delete, style: 'destructive', onPress: async () => { setBusy('delete'); setMessage(null); try { await deleteCurrentAccount(); } catch { setMessage(c.deleteFail); setBusy(null); } } },
    ]);
  };
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.status}><View style={[styles.dot, { backgroundColor: isSupabaseConfigured ? colors.primary : colors.accent }]} /><View style={{ flex: 1 }}><Text style={styles.statusTitle}>{isSupabaseConfigured ? 'Supabase' : 'Demo'}</Text><Text style={styles.statusText}>{isSupabaseConfigured ? c.secure : c.demo}</Text></View></View>
      <View style={styles.menu}>
        ...
        {showPreferences ? <View style={styles.preferences}>
          ...
          <View style={styles.options}>{([['metric',c.metric],['imperial',c.imperial]] as [UnitSystem,string][]).map(([value,label]) => <Pressable accessibilityRole="radio" accessibilityState={{ checked: unitSystem === value }} key={value} onPress={() => void setUnitSystem(value)} style={[styles.option, unitSystem === value && styles.optionSelected]}><Text style={[styles.optionText, unitSystem === value && styles.optionTextSelected]}>{label}</Text></Pressable>)}</View>
