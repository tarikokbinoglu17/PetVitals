...
const I = {
  tr: {
    ...
    symptomReason:(n:number)=>`90 günde ${n} semptom kaydı`, symptomTitle:'Tekrarlayan semptom örüntüsü', symptomMessage:(n:number)=>`Son 90 günde ${n} semptom kaydı girildi. Pawly tekrar eden bir örüntü tespit etti.`,
  ...
  en: {
    ...
    symptomReason:(n:number)=>`${n} symptom logs in 90 days`, symptomTitle:'Recurring symptom pattern', symptomMessage:(n:number)=>`${n} symptom entries were logged in the last 90 days. Pawly detected a repeating pattern worth discussing with a veterinarian.`,
  ...
  de: {
    ...
    symptomReason:(n:number)=>`${n} Symptom-Einträge in 90 Tagen`, symptomTitle:'Wiederkehrendes Symptommuster', symptomMessage:(n:number)=>`In den letzten 90 Tagen wurden ${n} Symptom-Einträge erfasst. Pawly hat ein wiederkehrendes Muster erkannt.`,
  ...
  es: {
    ...
    symptomReason:(n:number)=>`${n} registros de síntomas en 90 días`, symptomTitle:'Patrón de síntomas recurrentes', symptomMessage:(n:number)=>`Se registraron ${n} síntomas en los últimos 90 días. Pawly detectó un patrón repetitivo.`,
  ...
  ja: {
    ...
    symptomReason:(n:number)=>`90日間の症状記録 ${n}件`, symptomTitle:'症状が繰り返されています', symptomMessage:(n:number)=>`過去90日間に症状が${n}回記録されています。Pawlyが繰り返しの傾向を検出しました。`,
...
export async function evaluateSmartHealthAlerts(petId:string, language:IntelligenceLanguage='en'):Promise<SmartHealthAlert[]>{
  ...
  const scoreCard=makeAlert('pawly-intelligence-score',severityForScore(score),`Pawly Intelligence · ${score}/100 · ${label}`,`${explanation} ${c.disclaimer}`,nowIso,'health_score');
