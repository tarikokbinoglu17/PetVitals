...
const I = {
  tr: {
    ...
    symptomReason:(n:number)=>`90 günde ${n} semptom kaydı`, symptomTitle:'Tekrarlayan semptom örüntüsü', symptomMessage:(n:number)=>`Son 90 günde ${n} semptom kaydı girildi. PetVitals tekrar eden bir örüntü tespit etti.`,
  ...
  en: {
    ...
    symptomReason:(n:number)=>`${n} symptom logs in 90 days`, symptomTitle:'Recurring symptom pattern', symptomMessage:(n:number)=>`${n} symptom entries were logged in the last 90 days. PetVitals detected a repeating pattern worth discussing with a veterinarian.`,
  ...
  de: {
    ...
    symptomReason:(n:number)=>`${n} Symptom-Einträge in 90 Tagen`, symptomTitle:'Wiederkehrendes Symptommuster', symptomMessage:(n:number)=>`In den letzten 90 Tagen wurden ${n} Symptom-Einträge erfasst. PetVitals hat ein wiederkehrendes Muster erkannt.`,
  ...
  es: {
    ...
    symptomReason:(n:number)=>`${n} registros de síntomas en 90 días`, symptomTitle:'Patrón de síntomas recurrentes', symptomMessage:(n:number)=>`Se registraron ${n} síntomas en los últimos 90 días. PetVitals detectó un patrón repetitivo.`,
...
function makeAlert(id:string,severity:'low'|'medium'|'high',title:string,message:string,nowIso:string,type='petvitals_intelligence'):SmartHealthAlert{return{id,alert_type:type,severity,title,message,status:'active',detected_at:nowIso}}
...
export async function evaluateSmartHealthAlerts(petId:string, language:IntelligenceLanguage='en'):Promise<SmartHealthAlert[]>{
  ...
  const scoreCard=makeAlert('petvitals-intelligence-score',severityForScore(score),`PetVitals Intelligence · ${score}/100 · ${label}`,`${explanation} ${c.disclaimer}`,nowIso,'health_score');
...
export async function askPetHealthBrain(petId:string,question:string):Promise<HealthBrainAnswer>{const trimmedQuestion=question.trim();if(!trimmedQuestion)throw new Error('Question is required.');const client=requireSupabase();const{data,error}=await client.functions.invoke('pet-health-assistant',{body:{petId,question:trimmedQuestion}});if(error)throw error;if(data?.code==='PRO_REQUIRED')throw new Error('PetVitals Pro is required for AI Health Brain.');if(data?.code==='AI_NOT_CONFIGURED')throw new Error('AI Health Brain is not configured yet.');if(data?.error)throw new Error(String(data.error));if(typeof data?.answer!=='string'||!data.answer.trim())throw new Error('AI Health Brain returned an empty response.');return{answer:data.answer.trim(),disclaimer:typeof data.disclaimer==='string'?data.disclaimer:undefined};}
